...
write_file "$SUB/tfc_wood.json" "TFC wood substitutions: vanilla -> look-alike TFC wood (spruce->chestnut, birch->douglas_fir, jungle->spruce, dark_oak->hickory, cherry->kapok, bamboo->palm; oak/acacia/mangrove keep their name), across all forms incl. chest/workbench. Plus per-form candidate pools so the player can pick any TFC wood. The nether woods (crimson/warped) are handled by the optional Beneath datapack instead. Generated by gen_tfc_substitutions.sh."
