...
for form in planks log wood stripped_log stripped_wood leaves stairs slab fence fence_gate door trapdoor button pressure_plate chest trapped_chest workbench sign wall_sign hanging_sign wall_hanging_sign lectern decorative_bookshelf barrel; do
...
# ALABASTER. Purpur -> firmavanilla alabaster tiles/pillars (default purple, pick any colour). End stone ->
# uncolored TFC raw alabaster; end-stone bricks -> uncolored TFC alabaster bricks (each + pick-any-colour pool).
...
# TFC raw/bricks pools include the uncolored base (the end-stone default), so the GUI offers the pool on it.
for form in raw bricks; do
  { echo "tfc:alabaster/$form"; for c in $ALAB_COLORS; do echo "tfc:alabaster/$form/$c"; done; } | emit_tag "$TAGS/alabaster/$form.json"
done
...
fixed "minecraft:end_stone"        "tfc:alabaster/raw"
pool  "mctfc:subst/alabaster/raw"
fixed "minecraft:end_stone_bricks" "tfc:alabaster/bricks"
pool  "mctfc:subst/alabaster/bricks"
write_file "$SUB/firmavanilla_alabaster.json" "Purpur -> firmavanilla alabaster tiles/pillars (default purple, pick any colour). End stone -> uncolored TFC raw alabaster; end-stone bricks -> uncolored TFC alabaster bricks (each + a pick-any-colour pool that includes the uncolored base, since the GUI keys off the resolved default). Generated by gen_tfc_substitutions.sh."
