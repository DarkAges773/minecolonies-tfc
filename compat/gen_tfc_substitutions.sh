...
# AFC plank-suffixed forms at priority 1 (no hanging signs — AFC has none)
afc_plank_forms() {
  local v=$1 t=$2
...
# a full AFC wood mapping: planks + log/wood + stripped + the plank forms (priority 1)
...
write_file "$AFC_SUB/afc.json" "AFC-only substitutions (this whole datapack loads only when the 'afc'/ArborFirmaCraft mod is present, see AfcDataPack). Vanilla wood -> AFC wood (birch->eucalyptus, spruce->cypress, cherry->fig, acacia->baobab, jungle->teak) at priority 1, overriding the base TFC wood mapping (tfc_wood.json, priority 0) when AFC is present. Covers all forms except hanging signs (AFC has none; those keep the base TFC mapping). Generated by gen_tfc_substitutions.sh."

# AFC woods JOIN the per-form candidate pools (additive replace:false, only loaded with AFC), so any wood can be
# re-picked to an AFC wood and vice-versa. Only forms AFC has (no hanging signs). The base candidate rules already
# cover the merged tags.
for form in planks log wood stripped_log stripped_wood chest trapped_chest lectern; do
