...
wood_map() { case "$1" in
  ...
  spruce) echo pine;; birch) echo douglas_fir;; jungle) echo palm;;
  dark_oak) echo hickory;; cherry) echo kapok;; esac; }
...
# (vanilla bamboo is intentionally NOT mapped — TFC has no bamboo, and forcing it onto another TFC wood looked
#  wrong; bamboo blocks build as vanilla bamboo.)
...
write_file "$SUB/tfc_wood.json" "TFC wood substitutions: vanilla -> look-alike TFC wood (spruce->pine, birch->douglas_fir, jungle->palm, dark_oak->hickory, cherry->kapok; oak/acacia/mangrove keep their name), across all forms incl. chest/workbench, signs/wall-signs, and hanging/wall-hanging signs (copper metal default). Vanilla bamboo is intentionally not mapped (TFC has no bamboo). Plus per-form candidate pools so the player can pick any TFC wood. The nether woods (crimson/warped) are handled by the optional Beneath datapack instead. Generated by gen_tfc_substitutions.sh."
