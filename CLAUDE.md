...
| `:firmavanilla` | `firmavanilla` | `com.firmavanilla` | **Standalone** TerraFirmaCraft companion shipping TFC-palette decorative building blocks: the non-falling "cemented" cobblestone twins (migrated here from `:compat`) and statically-registered **chiseled sandstone** in TFC's 7 sand colours (the form TFC lacks). Depends **ONLY on TFC** — no MineColonies, no `:replacements`; it's pure content, usable by any TFC player. Block textures are machine-generated by a **.NET tool** (`tools/generate-textures`, ImageSharp). `:compat` hard-depends on it and points substitution rules at its blocks. See [docs/firmavanilla.md](docs/firmavanilla.md). |
