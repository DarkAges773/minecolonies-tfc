 ...
 * picker scoped to the miner's mineshaft palette ({@link MineshaftChoiceContext}). Installed by
 * {@code MixinSettingsModuleWindow} (miner only) via {@code @ModifyArg} on the list's {@code setDataProvider}
 * call, so the real settings rows keep rendering through MineColonies' own provider ({@link #delegate}).
 *
...
public class MineshaftSettingsListProvider implements ScrollingList.DataProvider
{
    ...
    private void renderButtonRow(final Pane rowPane)
    {
        ...
        // Action control — our replace icon over MineColonies' mini-button frame, the same icon-button style
