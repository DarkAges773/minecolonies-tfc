...
- **Stone** ([tfc_stone.json](../compat/src/main/resources/data/mctfc/block_substitutions/tfc_stone.json)): vanilla
  ...
  **granite/diorite/andesite** (which are real TFC rock types) map to the **same** rock — plain → `tfc:rock/raw/<rock>`,
  ...
  `raw`/`smooth` pools.
- **Rock-type palette presets** ([block_substitution_presets/](../compat/src/main/resources/data/mctfc/block_substitution_presets/),
  generated by [gen_tfc_presets.sh](../compat/gen_tfc_presets.sh)): one built-in [preset](replacements-internals.md#palette-presets-named-reusable-pick-sets)
  per TFC rock (20 — Granite, Basalt, Slate, …), surfaced read-only in Palette Swap's *Presets* picker. Each maps
  every **default-reachable** rock form (`{dacite, granite, diorite, andesite}` — the only rocks a vanilla
  blueprint resolves to, hence the only override keys) → the target rock's equivalent across every form
  (raw/cobble/bricks/smooth/mossy/cracked/chiseled + stairs/slabs/walls, plus magma for the 7 igneous rocks). The
  generator derives picks straight from the `subst/rock/*` pool tags, so every pick is a valid pool member and
  magma is skipped for non-igneous targets. Because picks are shared across buildings, loading one re-styles a
  whole colony onto the local stone in a click. Presets are read-only but clone-and-edit in the picker.
