...
- **Stone** ([tfc_stone.json](../compat/src/main/resources/data/mctfc/block_substitutions/tfc_stone.json)): vanilla
  ...
  **granite/diorite/andesite** (which are real TFC rock types) map to the **same** rock — plain → `tfc:rock/raw/<rock>`,
  ...
  `raw`/`smooth` pools.
- **Built-in palette presets** ([block_substitution_presets/](../compat/src/main/resources/data/mctfc/block_substitution_presets/),
  generated by [gen_tfc_presets.sh](../compat/gen_tfc_presets.sh)): read-only
  [presets](replacements-internals.md#palette-presets-named-reusable-pick-sets) under a **Built-in** folder in
  Palette Swap's *Presets* picker, in four groups — **rocks** (one per TFC rock, 20), **planks** + **logs** (one
  per wood, the plank vs log family kept separate), and **dirt** (one per TFC soil type). Each maps every
  **default-reachable** source form — the only blocks a vanilla blueprint resolves to, hence the only override
  keys: `{dacite, granite, diorite, andesite}` for rock, the 9 TFC woods vanilla planks/logs convert to for wood,
  `loam` for soil — → the target material's equivalent across every form (so a rock preset covers
  raw/cobble/bricks/smooth/…/walls + igneous magma; a wood preset covers planks/stairs/…/signs or log/wood/stripped).
  The generator derives picks straight from the `subst/*` pool tags, so every pick is a valid pool member (which
  also means each passes Palette Swap's server-side choice validation), and magma is skipped for non-igneous targets.
  **AFC and Beneath woods** get their own wood presets (TFC sources → that mod's wood), emitted into the respective
  optional datapacks so they load only with the mod, sharing the `mctfc` namespace + `builtin/planks|logs` folders.
  Because picks are shared across buildings, loading one re-styles a whole colony in a click. Read-only, but
  clone-and-edit in the picker.
