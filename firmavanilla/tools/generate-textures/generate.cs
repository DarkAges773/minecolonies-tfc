...
// Shared merge-tags written ONCE at the end (a mod can only ship one file per tag path): blocks that several
// sections contribute to. minecraft:mineable/shovel + tfc:can_landslide get prismarine deposits AND coarse dirt.
var shovelMineable = new List<string>();
var canLandslide = new List<string>();
string IdsTagBody(IEnumerable<string> ids) =>
    "{\"replace\":false,\"values\":[" + string.Join(",", ids.Select(id => "\"" + id + "\"")) + "]}";

// --- prismarine gravel deposits (TFC-style pan/sluice source for the vanilla prismarine family) -----------
// TFC leaves vanilla prismarine blocks + recipes intact but never generates guardians/monuments, so prismarine
// SHARDS and CRYSTALS are the missing inputs. One deposit block per TFC rock — firmavanilla:deposit/prismarine/
// <rock> — mirrors TFC's native_copper deposits exactly: a tfc:soil_disc swaps deep-ocean floor gravel
// (tfc:rock/gravel/<rock>) for the matching deposit; the player mines it then PANS or SLUICES it (TFC's
// data-driven panning/sluicing managers, under our namespace) for shards (common) + crystals (rare). The vanilla
// recipes then build the family. The crystal sheen is ONE hand-made overlay (the tracked, ANIMATED
// prismarine_overlay.png in the tool root — a 16x64 four-frame strip) layered over each rock's gravel by the
// tfc:block/ore parent model — NO per-rock art. The generator copies that input verbatim to the deposit texture
// path and writes its .mcmeta (the input stays in the tool root, so the copy never clobbers it). All JSON below
// is built with plain raw strings + .Replace so JSON braces need no escaping. See PrismarineDeposits.java.
int prismarine = 0;
{
    string A(params string[] p) => Path.Combine(new[] { resRoot, "assets", MODID }.Concat(p).ToArray());
    string D(params string[] p) => Path.Combine(new[] { resRoot, "data", MODID }.Concat(p).ToArray());
...
// --- soul lamps (teal "soul" recolor of TFC's lamp glass) -------------------------------------------------
// Vanilla-soul-lantern flavour for TFC's metal lamps. Visuals only here: hue-rotate TFC's lit/unlit lamp GLASS
// toward soul-fire teal (the #lamp texture only — the metal frame stays each metal's own), and reuse TFC's lamp
// geometry (parents tfc:block/lamp + lamp_hanging). One shared glass pair serves all 9 metals. The behaviour
// (reuse TFC's lamp block-entity, light 10, convert/burn-out) is code-side in SoulLamps.java.
int soulLamps = 0;
{
...
// --- coarse dirt textures (overlay = coarse_dirt − dirt, composited onto each TFC dirt) ----------------------
// TEXTURES ONLY (no block/blockstate/model/recipe yet). First DERIVE an overlay that isolates the "coarse" detail
// — the bits where vanilla coarse_dirt differs from plain dirt (the scattered darker pebbles): rgb = coarse_dirt's
// colour, alpha = how much it differs from plain dirt (scaled by COARSE_OVERLAY_GAIN), so it's transparent where
// they match. Save that overlay to the TOOL ROOT next to the masks/LUTs (tracked, inspectable/tweakable), then
// source-over it onto each TFC dirt so every soil keeps its own colour and just gains the pebbles. Inputs:
// input/vanilla/{coarse_dirt,dirt}.png + input/tfc/dirt/<soil>.png.
{
    ...
    var soils = new[] { "loam", "sandy_loam", "silt", "silty_loam" };
...
// Shared merge-tags (several sections contribute), written once: deposits + coarse dirt.
